...
module.exports = {
        ...
        f()
            ...
            .catch(function(error) {
              ...
              if( error.statusCode == 429 ) {
                _this.sDebug("'Too many requests' received, sleeping 60 seconds");
                _this.sDebug("Error: " + error);
                setTimeout( doCall, 60000 );
              } else
                reject( error );
            });
      };
      //return setTimeout( doCall, 5000 );
      return doCall();
    });
  },
  ...
  getEnvFiles: function(S, regionName, stage) {
    let regions;

    if (regionName != 'all' || stage == 'local') {  //single region
      if (stage == 'local') regionName = 'local';
    ...
    return BbPromise.map(regions, (region => {
      return this.getEnvFileAsMap(S, region, stage)
        .then(envVars => ({region, vars: envVars.map, raw: envVars.raw}))
    }));
  },
  ...
  getEnvFileAsMap: function(S, region, stage) {

    let deferred;

    if (stage == 'local') {
      deferred = fse.readFileAsync(S.getProject().getRootPath( '.env' ));
    } else {
      let Bucket     = S.getProject().getVariables().projectBucket,
        bucketRegion = S.getProject().getVariables().projectBucketRegion,
        Key          = ['serverless', S.getProject().name, stage, region, 'envVars', '.env'].join('/');

      SCli.log(`Getting ENV file from S3 bucket: ${Bucket}`);

      deferred =  S.getProvider().request('S3', 'getObject', {Bucket, Key}, stage, bucketRegion)
        .then((data) => data.Body || '');
    }

    return deferred
      .then((raw) => ({raw, map: dotenv.parse(raw)}))
      .catch(function(err) {
